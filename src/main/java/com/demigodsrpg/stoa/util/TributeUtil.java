...
public class TributeUtil {
    ...
    public static void save(String category, Material material, int amount) {
        ...
        db.insert(new TributeModel(category, material, amount));
        db.close();
    }

    public static void remove(String category, Material material) {
        if (find(category, material) != null) {
            find(category, material).remove();
        }
    }

    public static TributeModel find(String category, Material material) {
        TributeModel alias = new TributeModel();
        Db db = StoaServer.openDb();
        try {
            return db.from(alias).where(alias.category).is(category).and(alias.material).is(material).selectFirst();
        } finally {
            db.close();
        }
    }

    public static List<TributeModel> find(String category) {
        TributeModel alias = new TributeModel();
        Db db = StoaServer.openDb();
        try {
            return db.from(alias).where(alias.category).is(category).select();
        } finally {
            db.close();
        }
    }
    ...
    public static Map<Material, Integer> getTributeValuesMap() {
        ...
        for (TributeModel data : db.from(alias).select()) {
    ...
    public static int getTributes(Material material) {
        TributeModel data = find(getCategory(material), material);
    ...
    public static void saveTribute(ItemStack item) {
        TributeModel data = find(getCategory(item.getType()), item.getType());
