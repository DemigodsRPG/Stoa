...
public class StoaCharacterMeta extends DataAccess<UUID, StoaCharacterMeta>
{
	private static boolean LEVEL_SEPERATE_SKILLS = Stoa.getMythos().levelSeperateSkills();
	...
	public StoaCharacterMeta()
	{
	}

	@DataProvider(idType = DefaultIdType.UUID)
	public static StoaCharacterMeta of(UUID id, ConfigurationSection conf)
	{
		StoaCharacterMeta meta = new StoaCharacterMeta();
		meta.id = id;
		meta.favor = conf.getInt("favor");
		meta.maxFavor = conf.getInt("maxFavor");
		meta.skillPoints = conf.getInt("skillPoints");
		meta.notifications = Sets.newHashSet(conf.getStringList("notifications"));
		meta.character = UUID.fromString(conf.getString("character"));
		if(conf.getConfigurationSection("skillData") != null) meta.skillData = conf.getConfigurationSection("skillData").getValues(false);
		if(conf.getConfigurationSection("binds") != null) meta.binds = conf.getConfigurationSection("binds").getValues(false);
		if(conf.getConfigurationSection("warps") != null) meta.warps = conf.getConfigurationSection("warps").getValues(false);
		if(conf.getConfigurationSection("invites") != null) meta.invites = conf.getConfigurationSection("invites").getValues(false);
		return meta;
	}
	...
	public void setCharacter(StoaCharacter character)
	{
	...
	{
		warps.put(name.toLowerCase(), StoaLocation.track(location).getId().toString());
		save();
	}
	...
	{
		getInvites().put(name.toLowerCase(), StoaLocation.track(location).getId().toString());
		save();
	}
	...
	private static final DataAccess<UUID, StoaCharacterMeta> DATA_ACCESS = new StoaCharacterMeta();

	public static StoaCharacterMeta get(UUID id)
	{
		return DATA_ACCESS.getDirect(id);
	}

	public static Collection<StoaCharacterMeta> all()
	{
		return DATA_ACCESS.allDirect();
	}

	public static StoaCharacterMeta create(StoaCharacter character)
	{
		StoaCharacterMeta charMeta = new StoaCharacterMeta();
		charMeta.initialize();
