...
public class StoaCharacter extends DataAccess<UUID, StoaCharacter> implements Participant {
    ...
    public static StoaCharacter of(UUID id, ConfigurationSection conf) {
        ...
        if (conf.isString("enderInventory"))
            character.enderInventory = UUID.fromString(conf.getString("enderInventory"));
        if (conf.isList("deaths")) character.deaths = Sets.newHashSet(conf.getStringList("deaths"));
        if (conf.isList("potionEffects"))
            character.potionEffects = Sets.newHashSet(conf.getStringList("potionEffects"));
        return character;
    }
    ...
    public void remove() {
        ...
        if (getBukkitOfflinePlayer().isOnline() && stoaPlayer.getCharacter().getName().equalsIgnoreCase(getName()))
            stoaPlayer.setToMortal();
