...
import com.demigodsrpg.stoa.model.*;
...
public class CharacterController extends Controller<CharacterModel> implements Participant<CharacterModel> {
    ...
    public Collection<StoaStructureModel> getOwnedStructures(final String type) {
        return StoaStructureModel.find(new Predicate<StoaStructureModel>() {
            @Override
            public boolean apply(StoaStructureModel data) {
                return data.getTypeName().equals(type) && data.getOwner().equals(model.id());
            }
        });
    }
    ...
    public int getFavorRegen() {
        ...
        Db db = StoaServer.openDb();
        SkillModel found = Iterables.getFirst(db.from(alias).where(alias.type).is(SkillModel.Type.FAVOR_REGEN).and(alias.character).is(model.id()).select(), null);
