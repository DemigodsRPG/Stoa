...
public interface Ability
{
	...
	public static class Util
	{
		...
		public static boolean preProcessAbility(Player player, Ability ability)
		{
			...
			else return StoaCharacter.isCooledDown(character, ability.getName());
		}
		 ...
		 */
		public static void postProcessAbility(StoaCharacter character, Ability ability)
		{
			if(ability.getDelay() > 0) StoaCharacter.setCooldown(character, ability.getName(), ability.getDelay());
		...
		public static LivingEntity autoTarget(Player player)
		{
			...
			final StoaCharacter looking = StoaCharacter.of(player);
			...
			while(iterator.hasNext())
			{
				...
				targets.addAll(Collections2.filter(player.getNearbyEntities(range, range, range), new Predicate<Entity>()
				{
					...
					public boolean apply(Entity entity)
					{
						...
						{
							if(entity instanceof Tameable && ((Tameable) entity).isTamed() && StoaTameable.of((LivingEntity) entity) != null)
							{
								StoaTameable wrapper = StoaTameable.of((LivingEntity) entity);
								if(looking.alliedTo(wrapper)) return false;
							}
							else if(entity instanceof Player && StoaPlayer.of(((Player) entity)).isACharacter())
							{
		 ...
		 */
		public static Location adjustedAimLocation(StoaCharacter character, Location target)
		{
		...
		public static void dealDamage(LivingEntity source, LivingEntity target, double amount, EntityDamageEvent.DamageCause cause)
		{
			...
			{
				StoaPlayer owner = StoaPlayer.of(((Player) source));
				if(owner != null)
				{
