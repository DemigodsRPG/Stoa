...
public class TributeData extends DataAccess<UUID, TributeData>
{
	...
	public static TributeData of(UUID id, ConfigurationSection conf)
	{
		TributeData data = new TributeData(null);
		data.id = id;
		data.category = conf.getString("category");
		data.material = Material.valueOf(conf.getString("material"));
		data.amount = Integer.parseInt(conf.getString("amount"));
		return data;
	}
